...
- **`miyukitakilaranahtarlik/`** is a [Sveltia CMS](https://github.com/sveltia/sveltia-cms) panel loaded from unpkg (folder name is intentionally obscured to discourage bot scanning — previously `admin/`). It has its own, more permissive CSP in two places: the `ADMIN_CSP` constant in `_worker.js` (path-matched on `/miyukitakilaranahtarlik` and `/miyukitakilaranahtarlik/*`) and `miyukitakilaranahtarlik/index.html`'s meta tag. `miyukitakilaranahtarlik/config.yml` defines a `products` collection that writes markdown files (YAML frontmatter) into a `products/` folder, with media in `images/products/`.
...
- When changing **CSP**, update both the relevant constant in `_worker.js` (`PUBLIC_CSP` for the site, `ADMIN_CSP` for `/miyukitakilaranahtarlik/*`) **and** the `<meta http-equiv="Content-Security-Policy">` tag in the affected HTML file (`index.html` or `miyukitakilaranahtarlik/index.html`). Do **not** add a `Content-Security-Policy` line back to `_headers` — `_worker.js` actively deletes any upstream CSP to prevent a second, more restrictive policy from being merged in by the browser. Other security headers (HSTS, X-Frame-Options, etc.) live in `_worker.js` and `_headers` is for non-CSP additions only.
- New product categories or colors must be added in three places to stay consistent: the `products` array (`cat`/`colors` fields), the category/color filter buttons in the markup, and the matching `select` options in `miyukitakilaranahtarlik/config.yml`. The AI assistant's `AI_COLORS` map and intent parser may also need updates.
